...
int main(int argc, char *argv[]) {
    ...
    // 3. Établissement d'une connexion avec le serveur
    connect(sockfd, (struct sockaddr*)&server_addr, sizeof(server_addr));

    // 4. Envoi de données au serveur

    send(sockfd, message, strlen(message), 0);

    // 5. Réception d'un message de confirmation du serveur

    char buffer[TAILLEMAXMSG];
    read(sockfd, buffer, TAILLEMAXMSG);
